...
function getCronExpressions() {
  const first = (process.env.SCHEDULER_CRON_1 || "0 11 * * *").trim();
  const second = (process.env.SCHEDULER_CRON_2 || "0 14 * * *").trim();
  return [first, second];
}

// Define o nome exibido no historico para cada horario agendado.
function getSlotLabel(index, expression) {
  if (index === 0) return "11:00";
        ...
        try {
          ...
          logger.sucesso(
            `Coleta ${slotLabel} finalizada. Coamo: ${
              ...
              result.cvale.ok ? `ok (${result.cvale.count} itens)` : `falhou (${result.cvale.error})`
